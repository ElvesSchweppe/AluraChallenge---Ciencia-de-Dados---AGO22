...
/*Utilizarei uma técnica mais pedagógica de anotações inspirada no 5w2h se baseia em registrar antes de cada ação "O QUE?" "POR QUE?" eu chamo de "OQPQ" opcionalmente podemos registrar o "COMO?"*/

## PASSO 1 - Criei o Banco de dados 
/*OQPQ 1
O QUE - Criar o Banco de dados dentro do MySQL (importante ter o mesmo nome do banco a ser importado).
PORQUE - Para que possamos alocar e trabalhar com os dados que serao importados.
COMO - através de comando "CREATE DATABASE IF NOT EXISTS"
*/
...
/*OQPQ 2
O QUE - Importar e alocar os dados no banco criado.
PORQUE - Para ter acesso e trabalhar com os dados.
COMO - utilizei os menus e atalhos Server>Data Import.
*/

## PASSO 3 - Verifiquei superficialmente os dados nas tabelas
/*OQPQ 3
O QUE - Após a importação verificar as tabelas e os dados registrados
PORQUE - É importante garantir a existencia e reconhecer o tipo de dados que estamos trabalhando.
COMO - Através de comando para a exibição (SELECT FROM) das tabelas presentes (verifiqueis os nomes em 'Tables' no 'Navigator')
*/
...
/*OQPQ 4
O QUE - Reconhecer o tamanho dos bancos de dados
PORQUE - Saber a quantidade de dados em que se está trabalhado pode ser essencial na definição de estratégias de ação
COMO - Usei o metodo COUNT(*)
*/
SELECT(
...
/*OQPQ 5
O QUE - Verificar se há campos 'nulos' ou 'vazios'
PORQUE - Na proposta em questão é preciso que haja integridade nas informações para que possamos chegar em um modelo preditivo.
COMO - Usei o metodo COUNT(*) onde as colunas são nulas ('IS NULL') e onde são vazias "=''"
*/
...
## Passo 6.1 - dados_mutuarios (procurando ' ' - vazios não nulos)
SELECT(
...
/*OQPQ 6
O QUE - Além de dados inexistentes as tabelas podem conter dados inúteis ou suspeitos
PORQUE - Para que tenhamos um bom modelo preditivo precisamos 'limpar' as bases que serão usadas para teste e treino, do contrario poderemos fazer previsões enviesadas ou equivocadas.
COMO - Delimitei o que seriam dados suspeitos em cada tabela e utilzei o count.
*/
...
/*OQPQ 7
O QUE - Indicar as linhas com dados faltantes ou suspeitos para decisão sobre correção ou exclusão posteriores;
PORQUE - Para 'limpar' a base de dados e eliminar viés, ou falsas conclusões;
COMO - Defini uma função com base nas quatro inconscistências conceituais possíveis e usei um método de seleção e concatenação com base em um modelo de verificação exponencial (estilo somatório mas com base 10).
*/

DELETE FROM ids where ids.person_id = ''; /*removidas as 4 chaves*/

## Para facilitar o processo criei uma função que vai associar os campos a serem observados na presença de dados 'estranhos' na coluna de observações
DELIMITER $$
...
/*OQPQ 8
O QUE - Tradução dos dados da tabela e exportação para o CSV ;
PORQUE - Para dar mais inteligibilidade aos dados e gerar um arquivo para o processo de Machine Learning - que ocorrerá no Python;
COMO - Utilizei o metodo replace para as traduções nas colunas em que se fazi necessário, e exportei com o .
*/
...
## PASSO 12 - EXPORT CSV
## Exportei através do botão de atalho 'Export' no 'Result Grid' do MySQL Workbench